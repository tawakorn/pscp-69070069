...
```text
โจทย์จะให้รับค่า input วันและเดือน เป็นจำนวนเต็มมา ซึ่งบรรทัดที่หนึ่่งเป็นเดือน และสองเป็นวัน โดยโจทย์จะกำหนดเงื่อนไขว่า
...
จะมีเงื่อนไขที่ว่าทุกๆเดือนที่ หาร 3 ลงตัวนั่นคือ 3 6 9 12 ฤดูกาลนั้นจะเริ่มเปลี่ยนตั้งแต่วันที่ 21 ซึ่งวิธีเดียวที่ผมคิดออกคือการใช้ if else

Constraints:
โจทย์ข้อนี้มี Constraints คือ Input มี 2 บรรทัด เป็นจำนวนเต็ม บรรทัดที่หนึ่่งเป็นเดือน อันที่สองเป็นวัน 
```
...
```text
Step 1: ผมดูจากโจทย์ว่าต้องการ input แบบใด ซึ่งคือ int เดือน และ วัน
Step 2: ผมเริ่มสร้างเงื่อนไข if else ครั้งนี้ผมได้ความรู้มาใหม่จากการทำโจทย์ในด้านนอก คือการใช้ เซ็ต {} ผมเลยทำเดือนที่ไม่มีการเปลี่ยนเอาใส่เซ็ต เช่น if MONTH in {1,2} เผื่อความกระชับ
Step 3: และสำหรับเดือนที่หารสามลงตัว ผมจะทำเช็คซ้ำ if else ไปอีกขั้นด้านในว่า หาก DAY > 21: ให้ print ฤดูกาลของเดือน
Step 4: ยพรืะ ชื่อฤดูกาลออกมาตามเงื่อนไข
...
```text
เหมือนกัน เพราะเป็นวิธีเดียวที่ผมนึกได้ในขณะนั้น แต่ในโค้ดอาจจะยังคงยาวเกินไปหน่อย แต่ยังคงถูกต้องตามโจทย์
```
...
```text
ผมอยากทดสอบกรณีที่มันดูทั่วไป และ ผมยังมีความสับสนเครื่องหมายเล็กน้อยเลยลองวันที่ 21 ว่าโค้ดมันจะนับไหม ซึ่งมันควรนับ
```
...
```text
ผมลองวิธีตามปกติหากว่ามันจะทำงานได้
```
...
```text
ผมลองในกรณีที่เกินวันที่ 21 แล้ว เผื่อมั่นใจว่าโค้ดทำงานได้ปกติ
```

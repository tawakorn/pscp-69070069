...
```text
input ใส่ string ต้นทางและปลายทาง
ต่อมาให้ใส่ น้ำหนักสินค้า

constraints คือ สถานที่ต้องตรงตามที่โจทย์กำหนดเท่านั้น หากนอกเหนือกว่านี้จะขึ้น Error
```
...
```text
Step1 : รับค่าสถานที่ต้นทางและปลายทางมาและรับน้ำหนักของสินค้า
Step2: นำเข้า if else เช็คว่าต้นทางและปลายทางอยู่ในที่เรากำหนดหรือไม่หากอยู่ให้ เข้าการคำนวณตามที่ตารางในโจทย์กำหนด หากไม่เข้าเลยให้พิมพ์ Error
Step3: print ผลลัพธ์
```
...
```text
เหมือนกันเลย ผมนำ weight มา * กับ tax ในตาราง ตรวจจับสถานที่ต้นทางและปลายทางจากนั้นให้เช็คว่าเงื่อนไขนี้ไปตรงกับเงื่อนไขใดแล้วคำนวณค่าออกมา นำทั้งweightax , tax มา + กัน
```
...
```text
ผมลองในกรณีที่เป็น case ทั่วไปเพื่อตรวจสอบว่าโค้ดทำงานได้ปกติ
```
...
```text
กรณีที่กรอกผิดพลาดไม่เข้าซักเงื่อนไขไปสถานที่เดิม
```

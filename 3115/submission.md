...
```text
Input: บรรทัดแรครับจำนวนร้านกับจำนวนรอบที่ต้องเช็ค ต่อมาเป็นเวลาเปิดปิดของแต่ละร้าน และบรรทัดสุดท้ายคือเวลาที่เราอยากตรวจ
Output: ปริ้นจำนวนร้านที่เปิดอยู่ ณ เวลานั้นๆ ออกมาในบรรทัดเดียว คั่นด้วยเว้นวรรค
Constraints: จำนวนร้านกับรอบตรวจมีได้เยอะสุด 1 แสน ส่วนเวลาเป็นหน่วยนาที ตั้งแต่0 ถึง1440
```
...
```text
Step 1: รับค่าจำนวนร้านและจำนวนรอบตรวจ
Step 2: วนลูปรับเวลาเปิด-ปิดของทุกร้านเก็บไว้
Step 3: รับค่าเวลาที่ต้องการตรวจทั้งหมด
Step 4: วนลูปเช็คทีละเวลาเทียบกับทุกร้าน
Step 5: ตรวจว่าเวลาเช็ค >= เวลาเปิด และ < เวลาปิด หรือไม่
Step 6: ถ้าจริง ให้นับบวก 1
Step 7: พิมพ์จำนวนที่นับได้ออกมาต่อกันในบรรทัดเดียว
```
...
```text
เหมือนเดิมเพราะน่าจะเป็นวิธีที่ดีที่สุด
```
...
```text
ตรวจสอบช่วงเวลาปกติที่มีร้านเปิดซ้อนทับกันหลายร้าน
```

...
```text
input คือรับเป็นตัวเลขอะไรก็ได้

output คือเอาเลขที่รับมานั้นมาดูว่าตัวไหนบ้างในเลขนั้นที่หารลงตัว ตั้งแต่ตัวเลขนั้นถึง 0
...
```text
Step 1: ผมดูจากโจทย์ว่าต้องการ input แบบใด ซึ่งคือ ตัวเลข int ทั่วไป
Step 2: ผมจะลองสร้าง loop ขึ้นมา โดยเป็น loop ที่เช็คตั้งแต่ตัวเลขที่รับมา -1 ไปเรื่อยๆจนเหลือ 0 
Step 3: เลขตัวไหนที่ % 10 แล้วได้ 0 ผมจะจับตัวเลขนั้นเข้า list ไว้
Step 4: เมื่อครบแล้วก็ print list
...
```text
เหมือนกัน เพราะเป็นวิธีเดียวที่ผมนึกได้ และน่าจะดีที่สุดแล้ว
```
...
```text
ลองเลขที่ไม่ใช่ค่าลงท้ายด้วยศูนย์ เพื่อดูว่าเริ่มหารถูกไหม
```
...
```text
ผมลองกรณีที่หาร 10 ลงตัวพอดี เพื่อดูว่ามันจะเเอาตัวที่กรอกเข้าไปนับไหม
```

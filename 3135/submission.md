...
```text
input : รับ N K T เป็น เลข 3ตัว
output : จำนวนคนที่ได้จับของขวัญทั้งหมด นับขโมยด้วยหากส่งถึง
constraints : ของเริ่มที่คนที่ 1 เสมอ แล้วส่งต่อไปทีละ K คนแบบวนเป็นวงกลม
```
...
```text
Step 1: รับinput เลข 3 N, K, T
Step 2: ดักทางก่อน ถ้าขโมยเป็นคนที่ 1 ให้ปริ้น 1 แล้วจบโปรแกรมเลย
Step 3: ตั้งค่าเริ่มต้นคือ ของอยู่ที่คนที่ 1 current = 1 และนับจำนวนคนจับเป็น 1 count = 1
Step 4: เปิดลูปวนไปเรื่อยๆ หาคนรับของคนถัดไปโดยเอา current + K
Step 5: ถ้าตำแหน่งคนรับมันเกินจำนวนคน N ให้เอาไปลบด้วย N เพื่อให้มันวนกลับมาเริ่มใหม่
Step 6: เช็คเงื่อนไขหยุด ถ้าวนกลับมาตกคนที่ 1 ให้ break loop
Step 7: ถ้ายาวไป ให้ count + 1
Step 8: เช็คต่อว่าตกที่ขโมยไหม ถ้าตกที่ขโมยพอดีให้ break loop
Step 9: print count
```
...
```text
เหมือนเดิมเพราะน่าจะเวิร์คนสุด
```
...
```text
เคสปกติจากโจทย์ ดูว่าทำงานปกติไหม
```
...
```text
ดักเคสคนที่ 1 เป็นขโมยซะเอง ควรจะตอบ 1 
```
